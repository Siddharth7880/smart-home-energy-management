...
public interface EnergyUsageLogRepository extends JpaRepository<EnergyUsageLog, Long> {
        ...
        /** Bulk JPQL delete — does NOT load entities, far more efficient for large datasets. */
        @Modifying
        @Transactional
        @Query("DELETE FROM EnergyUsageLog e WHERE e.device.id = :deviceId")
        void bulkDeleteByDeviceId(@Param("deviceId") Long deviceId);
        ...
        Object[] findPeakHour(@Param("deviceIds") List<Long> deviceIds,
                        @Param("startTime") LocalDateTime startTime, @Param("endTime") LocalDateTime endTime);
        ...
        @Query(value = "SELECT HOUR(e.timestamp) AS hr, SUM(e.energy_used) AS total " +
                        ...
                        "WHERE e.timestamp BETWEEN :startTime AND :endTime " +
                        "GROUP BY HOUR(e.timestamp) ORDER BY total DESC LIMIT 1", nativeQuery = true)
        Object[] findGlobalPeakHour(@Param("startTime") LocalDateTime startTime,
                        @Param("endTime") LocalDateTime endTime);
}
